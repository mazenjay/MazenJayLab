...
const FlapCell = React.memo(function FlapCell({
  ...
  stepMs: number;
  flipDuration: number;
  /** 为 true 时停在当前目标字符，不跑 scramble / Motion 翻页（样式与格子保留） */
  ...
  }, [target, delay, stepMs, animationsPaused]);
...
(prevProps, nextProps) =>
  ...
  prevProps.animationsPaused === nextProps.animationsPaused,
);
...
export interface TextFlippingBoardProps {
  ...
  duration?: number;
  /**
   * 为 true 时保留整面翻牌网格样式，但停止 scramble 与 3D 翻页动画（与 Lenis 并存时不抢主线程）。
   */
  animationsPaused?: boolean;
}
